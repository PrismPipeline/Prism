...
class ItemList(QDialog, ItemList_ui.Ui_dlg_ItemList):
	...
	def enableOk(self):
		self.buttonBox.button(QDialogButtonBox.Ok).setEnabled(len(self.tw_steps.selectedItems()) > 0)
	...
	def saveStep(self, abrev, name):
		psteps = ast.literal_eval(self.core.getConfig('globals', "pipeline_steps", configPath=self.core.prismIni))
		if abrev not in psteps:
			psteps[str(abrev)] = str(name)
			self.core.setConfig('globals', "pipeline_steps", str(psteps), configPath=self.core.prismIni)