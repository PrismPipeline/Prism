...
class Prism_PluginEmpty_Functions(object):
	...
	def prismSettings_loadUI(self, origin):
		...
		origin.gb_prjmanPrjIntegration.toggled.connect(lambda x: self.prismSettings_prjmanToggled(origin, x))
	...
	def prismSettings_loadPrjSettings(self, origin):
		...
		loadFunctions["prjmanActive"] = lambda x: origin.gb_prjmanPrjIntegration.setChecked(x)
		...
		loadFunctions["prjmanScriptName"] = lambda x: origin.e_prjmanScriptName.setText(x)
	...
	def prismSettings_savePrjSettings(self, origin):
		...
		saveData.append(['PluginEmpty', 'active', str(origin.gb_prjmanPrjIntegration.isChecked())])
		saveData.append(['PluginEmpty', 'site', str(origin.e_prjmanSite.text())])
		saveData.append(['PluginEmpty', 'projectname', str(origin.e_prjmanPrjName.text())])
		saveData.append(['PluginEmpty', 'scriptname', str(origin.e_prjmanScriptName.text())])
	...
	def pbBrowser_getMenu(self, origin):
		prjman = self.core.getConfig("PluginEmpty", "active", configPath=self.core.prismIni)
	...
	def pbBrowser_getAssetMenu(self, origin, assetname, assetPath):
		...
		if prjman is not None and eval(prjman) and pVersion == 2:
			...
			prjmanAct.triggered.connect(lambda: self.openprjman(assetname, eType="Asset", assetPath=assetPath))
			return prjmanAct
	...
	def assetCreated(self, origin, itemDlg, assetPath):
		if hasattr(itemDlg, "chb_createInPluginEmpty") and itemDlg.chb_createInPluginEmpty.isChecked():
	...
	def editShot_closed(self, origin, shotName):
		if hasattr(origin, "chb_createInPluginEmpty") and origin.chb_createInPluginEmpty.isChecked():
	...
	def pbBrowser_getPublishMenu(self, origin):
		prjman = self.core.getConfig("PluginEmpty", "active", configPath=self.core.prismIni)
		if prjman is not None and eval(prjman) and len(origin.seq) > 0 and pVersion == 2:
