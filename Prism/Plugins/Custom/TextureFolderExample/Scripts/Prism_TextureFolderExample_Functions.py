...
class Prism_TextureFolderExample_Functions(object):
	...
	def onAssetDlgOpen(self, origin, assetDialog):
		if hasattr(self, "textureFolders"):
			...
			assetDialog.rb_asset.toggled.connect(lambda x: assetDialog.chb_textureFolder.setEnabled(x))
	...
	def onAssetCreated(self, origin, assetName, assetPath, assetDialog=None):
		if hasattr(self, "textureFolders"):
			...
			if assetDialog is not None and hasattr(assetDialog, "chb_textureFolder") and assetDialog.chb_textureFolder.isChecked():

				# get the asset hierarchy
				relPath = assetPath.replace(self.core.getAssetPath(), "")[1:]

				# get the asset-texture-path
				folderPath = os.path.join(self.core.getTexturePath(), relPath, assetName)
				...
				if not os.path.exists(folderPath):
					try:
						...
						QMessageBox.warning(self.core.messageParent, "Prism", "Could not create the texture folder for asset %s" % assetName)