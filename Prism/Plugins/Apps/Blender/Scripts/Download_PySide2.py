...
def openSetupDialog():
    ...
    msg.setText(
        "Prism requires the PySide2 library for Python %s.%s in order to display its user interfaces.\n\nYou can download it now automatically or you can download it manually from the Prism website."
        % (sys.argv[-1][-2], sys.argv[-1][-1])
    )
...
""" % (
    ...
    if "failed" in str(stdOutData) or len(str(stdOutData).split(".")) < 3:
        QMessageBox.information(
            QWidget(),
            "Prism",
            "Unable to connect to www.prism-pipeline.com. Could not download PySide2.",
            QMessageBox.Ok,
        )
        return
    ...
    while True:
        ...
        if length:
            waitmsg.setText(
                dlMsg + " {}%\r done".format(int((size / float(length)) * 100))
            )
            QCoreApplication.processEvents()
    ...
    if os.path.exists(os.path.dirname(filepath)):
        shutil.rmtree(
            os.path.dirname(filepath), ignore_errors=False, onerror=handleRemoveReadonly
        )
    ...
    waitmsg = QMessageBox(
        QMessageBox.NoIcon, "Prism", "Installing - please wait..", QMessageBox.Cancel
    )
    waitmsg.buttons()[0].setHidden(True)
    waitmsg.show()
    QCoreApplication.processEvents()
    ...
    for i in dirs:
        ...
        if os.path.exists(installedLib):
            shutil.rmtree(
                installedLib, ignore_errors=False, onerror=handleRemoveReadonly
            )
    ...
    QMessageBox.information(
        QWidget(),
        "Prism",
        "Successfully set up PySide2 for Prism (%s)." % libStr,
        QMessageBox.Ok,
    )
