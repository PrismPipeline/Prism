...
def setAOVData(origin, node, aovNum, item):
	if item.column() == 0:
		origin.core.appPlugin.setNodeParm(node, "ar_aov_label" + aovNum, val=item.text())
	elif item.column() == 1:
		origin.core.appPlugin.setNodeParm(node, "ar_aov_exr_layer_name" + aovNum, val=item.text())


def getDefaultPasses(origin):
	aovs = origin.core.getConfig("defaultpasses", "houdini_arnold", configPath=origin.core.prismIni)
	if aovs is None:
		aovs = origin.core.appPlugin.arnoldPasses[2]
...
def addAOV(origin, aovData):
	...
	origin.core.appPlugin.setNodeParm(origin.node, "ar_aov_label" + str(passNum), val=aovData[0])
	origin.core.appPlugin.setNodeParm(origin.node, "ar_aov_exr_layer_name" + str(passNum), val=aovData[1])
...
def refreshAOVs(origin):
	...
	for i in range(origin.node.parm("ar_aovs").eval()):
		...
		passName = QTableWidgetItem(origin.node.parm("ar_aov_exr_layer_name" + str(i+1)).eval())
...
def executeAOVs(origin, outputName):
	if not origin.core.appPlugin.setNodeParm(origin.node, "ar_picture_format", val="exr"):
	...
	for i in range(origin.node.parm("ar_aovs").eval()):
		...
		if not origin.core.appPlugin.setNodeParm(origin.node, "ar_aov_separate" + str(i+1), val=True):
			return [origin.state.text(0) + ": error - Publish canceled"]
		if not origin.core.appPlugin.setNodeParm(origin.node, "ar_aov_separate_file" + str(i+1), val=passOutputName):
			return [origin.state.text(0) + ": error - Publish canceled"]
