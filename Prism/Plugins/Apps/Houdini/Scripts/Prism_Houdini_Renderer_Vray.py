...
def getDefaultPasses(origin):
	aovs = origin.core.getConfig("defaultpasses", "houdini_vray", configPath=origin.core.prismIni)
	if aovs is None:
...
def addAOV(origin, aovData):
	aovNode = origin.node.node(origin.node.parm("render_network_render_channels").eval())
	if aovNode is None:
	...
	ccNode = aovNode.createNode("VRayNodeRenderChannelColor", re.sub("[^0-9a-zA-Z\\.]+", "_", aovData[1]))
...
def executeAOVs(origin, outputName):
	...
	if not origin.core.appPlugin.setNodeParm(origin.node, "SettingsOutput_img_save", val=True):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "SettingsOutput_img_file_path", val=outputName):
		return [origin.state.text(0) + ": error - Publish canceled"]

	if not origin.core.appPlugin.setNodeParm(origin.node, "SettingsOutput_img_file_needFrameNumber", val=True):
		return [origin.state.text(0) + ": error - Publish canceled"]
