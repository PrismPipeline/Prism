...
def setAOVData(origin, node, aovNum, item):
	if item.column() == 0:
		...
		origin.core.appPlugin.setNodeParm(node, "RS_aovSuffix_" + aovNum, val=item.text())


def getDefaultPasses(origin):
	aovs = origin.core.getConfig("defaultpasses", "houdini_redshift", configPath=origin.core.prismIni)
	if aovs is None:
...
def addAOV(origin, aovData):
	...
	origin.core.appPlugin.setNodeParm(origin.node, "RS_aovID_" + str(passNum), val=typeID)
	origin.core.appPlugin.setNodeParm(origin.node, "RS_aovSuffix_" + str(passNum), val=aovData[1])
...
def refreshAOVs(origin):
	...
	for i in range(origin.node.parm("RS_aov").eval()):
		...
		passTypeName = QTableWidgetItem(origin.node.parm("RS_aovID_" + str(i+1)).menuLabels()[passTypeID])
		passName = QTableWidgetItem(origin.node.parm("RS_aovSuffix_" + str(i+1)).eval())
...
def executeAOVs(origin, outputName):
	...
	if not origin.core.appPlugin.setNodeParm(origin.node, "RS_outputFileNamePrefix", val=outputName):
		return [origin.state.text(0) + ": error - Publish canceled"]
	...
	for parm in origin.node.parms():
		if "RS_aovCustomPrefix" in parm.name():
			...
			if layerType == "Cryptomatte":
				layerName = "$AOV"
			commonOutPut = origin.node.parm("RS_outputFileNamePrefix").unexpandedString()
...
def setResolution(origin):
	...
	if not origin.core.appPlugin.setNodeParm(origin.node, "RS_overrideCameraRes", val=True):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "RS_overrideResScale", val="user"):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "RS_overrideRes1", val=origin.sp_resWidth.value()):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "RS_overrideRes2", val=origin.sp_resHeight.value()):
		return [origin.state.text(0) + ": error - Publish canceled"]
