...
def setAOVData(origin, node, aovNum, item):
	if item.column() == 0:
		origin.core.appPlugin.setNodeParm(node, "vm_channel_plane" + aovNum, val=item.text())
	elif item.column() == 1:
		origin.core.appPlugin.setNodeParm(node, "vm_variable_plane" + aovNum, val=item.text())


def getDefaultPasses(origin):
	aovs = origin.core.getConfig("defaultpasses", "houdini_mantra", configPath=origin.core.prismIni)
	if aovs is None:
...
def addAOV(origin, aovData):
	...
	origin.core.appPlugin.setNodeParm(origin.node, "vm_channel_plane" + str(passNum), val=aovData[0])
	origin.core.appPlugin.setNodeParm(origin.node, "vm_usefile_plane" + str(passNum), val=True)
	origin.core.appPlugin.setNodeParm(origin.node, "vm_variable_plane" + str(passNum), val=aovData[1])
...
def refreshAOVs(origin):
	...
	for i in range(origin.node.parm("vm_numaux").eval()):
		...
		passName = QTableWidgetItem(origin.node.parm("vm_channel_plane" + str(i+1)).eval())
		passVariable = QTableWidgetItem(origin.node.parm("vm_variable_plane" + str(i+1)).eval())
...
def executeAOVs(origin, outputName):
	...
	for i in range(origin.node.parm("vm_numaux").eval()):
		...
		passOutputName = os.path.join(os.path.dirname(os.path.dirname(outputName)), passName, os.path.basename(outputName).replace("beauty", passName))
		if not os.path.exists(os.path.split(passOutputName)[0]):
			os.makedirs(os.path.split(passOutputName)[0])

		if not origin.core.appPlugin.setNodeParm(origin.node, "vm_usefile_plane" + str(i+1), val=True):
			return [origin.state.text(0) + ": error - Publish canceled"]
		if not origin.core.appPlugin.setNodeParm(origin.node, "vm_filename_plane" + str(i+1), val=passOutputName):
			return [origin.state.text(0) + ": error - Publish canceled"]

		if passVar != "all":
			if not origin.core.appPlugin.setNodeParm(origin.node, "vm_channel_plane" + str(i+1), val="rgb"):
				return [origin.state.text(0) + ": error - Publish canceled"]
		else:
			if not origin.core.appPlugin.setNodeParm(origin.node, "vm_channel_plane" + str(i+1), val=""):
				return [origin.state.text(0) + ": error - Publish canceled"]
			if not origin.core.appPlugin.setNodeParm(origin.node, "vm_lightexport" + str(i+1), val=1):
				return [origin.state.text(0) + ": error - Publish canceled"]
...
def setResolution(origin):
	if not origin.core.appPlugin.setNodeParm(origin.node, "override_camerares", val=True):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "res_fraction", val="specific"):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "res_overridex", val=origin.sp_resWidth.value()):
		return [origin.state.text(0) + ": error - Publish canceled"]
	if not origin.core.appPlugin.setNodeParm(origin.node, "res_overridey", val=origin.sp_resHeight.value()):
		return [origin.state.text(0) + ": error - Publish canceled"]
...
def postExecute(origin):
	for i in range(origin.node.parm("vm_numaux").eval()):
		if not origin.core.appPlugin.setNodeParm(origin.node, "vm_channel_plane" + str(i+1), val=origin.passNames[i][0]):
			return [origin.state.text(0) + ": error - Publish canceled"]
