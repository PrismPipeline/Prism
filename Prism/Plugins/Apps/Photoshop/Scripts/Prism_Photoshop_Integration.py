...
class Prism_Photoshop_Integration(object):
	...
	def integrationAdd(self, origin):
		path = QFileDialog.getExistingDirectory(self.core.messageParent, "Select Photoshop folder", os.path.dirname(self.examplePath))
	...
	def writePhotoshopFiles(self, photoshoppath):
		try:
			if not os.path.exists(photoshoppath):
				QMessageBox.warning(self.core.messageParent, "Prism Integration", "Invalid Photoshop path: %s.\nThe path doesn't exist." % photoshoppath, QMessageBox.Ok)
				return False
			...
			exc_type, exc_obj, exc_tb = sys.exc_info()
			msgStr = "Errors occurred during the installation of the Photoshop integration.\nThe installation is possibly incomplete.\n\n%s\n%s\n%s" % (str(e), exc_type, exc_tb.tb_lineno)
	...
	def removeIntegration(self, installPath):
		try:
			for i in ["Prism - 1 Tools.jsx", "Prism - 2 Save version.jsx", "Prism - 3 Save comment.jsx", "Prism - 4 Export", "Prism - 5 ProjectBrowser.jsx", "Prism - 6 Settings.jsx"]:
			...
			msgStr = "Errors occurred during the removal of the Photoshop integration.\n\n%s\n%s\n%s" % (str(e), exc_type, exc_tb.tb_lineno)
			msgStr += "\n\nRunning this application as administrator could solve this problem eventually."
	...
	def installerExecute(self, photoshopItem, result, locFile):
		try:
			...
			if photoshopItem.checkState(0) == Qt.Checked and os.path.exists(photoshopItem.text(1)):
				result["Photoshop integration"] = self.writePhotoshopFiles(photoshopItem.text(1))
