...
class Prism_Pandora_Functions(object):
	def __init__(self, core, plugin):
		...
		uiPath = os.path.abspath(os.path.join(__file__, os.pardir, os.pardir, "UserInterfaces"))
		...
		if osFolder is not None and os.path.exists(osFolder):
			...
			if os.path.exists(jobDir):
				...
				for x in sorted(os.listdir(jobDir)):
					...
					jcode = self.Pandora.getConfig(cat="information", param="jobcode", configPath=os.path.join(jobDir, x))
	...
	def startSubmission(self, origin, jobOutputFile, parent):
		...
		if hasattr(origin, "curCam") and hasattr(self.core.appPlugin, "getCamName") and origin.curCam != "Current View":
	...
	def sm_render_managerChanged(self, origin):
		...
		getattr(self.core.appPlugin, "sm_render_managerChanged", lambda x,y: None)(origin, True)
