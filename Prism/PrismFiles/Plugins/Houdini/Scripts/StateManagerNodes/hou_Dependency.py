...
class DependencyClass(object):
	...
	def connectNode(self):
		if len(hou.selectedNodes()) > 0 and (hou.selectedNodes()[0].type().name() == "file"):
	...
	def preExecuteState(self):
		...
		if self.cb_manager.currentText() in self.core.rfManagers:
			warnings += self.core.rfManagers[self.cb_manager.currentText()].sm_dep_preExecute(self)
	...
	def executeState(self, parent):
		try:
			if self.cb_manager.currentText() in self.core.rfManagers:
				self.core.rfManagers[self.cb_manager.currentText()].sm_dep_execute(self, parent)
	...
	def getStateProps(self):
		...
		return {"statename":self.e_name.text(), "rjmanager":str(self.cb_manager.currentText()), "connectednode":str(curNode), "frameoffset":self.sp_offset.value(), "dependencies":str(self.dependencies), "stateenabled":str(self.state.checkState(0))}