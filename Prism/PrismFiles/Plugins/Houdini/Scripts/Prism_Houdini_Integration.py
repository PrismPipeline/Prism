...
class Prism_Houdini_Integration(object):
	def __init__(self, core, plugin):
		...
		if platform.system() == "Windows":
			self.examplePath = os.environ["userprofile"] + "\\Documents\\houdini16.5"
			if not os.path.exists(self.examplePath):
				oldpath = os.environ["userprofile"] + "\\Documents\\houdini16.0"
				if os.path.exists(oldpath):
					self.examplePath = oldpath
			...
			if not os.path.exists(self.examplePath):
				oldpath = os.path.join("/home", userName, "houdini16.0")
				if os.path.exists(oldpath):
					self.examplePath = oldpath
			...
			if not os.path.exists(self.examplePath):
				oldpath = "/Users/%s/Library/Preferences/houdini/16.0" % userName
				if os.path.exists(oldpath):
					self.examplePath = oldpath
