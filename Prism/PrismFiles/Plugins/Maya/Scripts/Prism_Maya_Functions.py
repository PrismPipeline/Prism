...
class Prism_Maya_Functions(object):
	...
	def sm_export_addObjects(self, origin):
		for i in cmds.ls( selection=True , long = True):
			if not i in origin.nodes:
				...
				cmds.sets(i, include=origin.l_taskName2.text())
	...
	def sm_export_setTaskText(self, origin, prevTaskName):
		origin.l_taskName2.setText(cmds.rename(prevTaskName, origin.nameWin.e_item.text()))


	@err_decorator
	def sm_export_removeSetItem(self, origin, node):
		cmds.sets(node, remove=origin.l_taskName2.text())


	@err_decorator
	def sm_export_clearSet(self, origin):
		cmds.sets( clear=origin.l_taskName2.text() )
	...
	def sm_export_updateObjects(self, origin):
		...
		try:
			...
			cmds.select(origin.l_taskName2.text())
		except:
			cmds.sets(name=origin.l_taskName2.text())
	...
	def sm_export_exportAppObjects(self, origin, startFrame, endFrame, outputName, scaledExport=False, nodes=None, expType=None):
		...
		if nodes is None:
			if not self.isNodeValid(origin, origin.l_taskName2.text()):
				return "Canceled: The selection set \"%s\" is invalid." % origin.l_taskName2.text()

			cmds.select(origin.l_taskName2.text(), noExpand=True)
	...
	def sm_export_preDelete(self, origin):
		try:
			cmds.delete(origin.l_taskName2.text())
		except:
			pass
	...
	def sm_playblast_createPlayblast(self, origin, jobFrames, outputName):
		...
		cmdString = "cmds.playblast( startTime=%s, endTime=%s, format=\"image\", percent=100, viewer=False, forceOverwrite=True, filename=\"%s\"" % (jobFrames[0], jobFrames[1], outputName.replace("\\", "\\\\"))
