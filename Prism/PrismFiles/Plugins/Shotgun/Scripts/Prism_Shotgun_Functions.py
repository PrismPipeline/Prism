...
class Prism_Shotgun_Functions(object):
	...
	def prismSettings_loadUI(self, origin):
		...
		origin.gb_sgPrjIntegration.toggled.connect(lambda x: self.prismSettings_sgToggled(origin, x))
	...
	def prismSettings_saveSettings(self, origin):
		...
		saveData.append(['shotgun', "sguseaccount", str(origin.gb_sgAccount.isChecked())])
		saveData.append(['shotgun', "sgusername", str(origin.e_sgUserName.text())])
		saveData.append(['shotgun', "sguserpassword", str(origin.e_sgUserPassword.text())])
	...
	def prismSettings_savePrjSettings(self, origin):
		...
		saveData.append(['shotgun', 'active', str(origin.gb_sgPrjIntegration.isChecked())])
	...
	def pbBrowser_getAssetMenu(self, origin, assetname, assetPath):
		...
		if sg is not None and eval(sg) and pVersion == 2:
			...
			sgAct.triggered.connect(lambda: self.openSg(assetname, eType="Asset", assetPath=assetPath))
			return sgAct
	...
	def createAsset_open(self, origin):
		...
		origin.w_prjManager.layout().insertWidget(0, origin.chb_createInShotgun)
	...
	def editShot_closed(self, origin, shotName):
		if hasattr(origin, "chb_createInShotgun") and origin.chb_createInShotgun.isChecked():
	...
	def connectToShotgun(self, user=True):
		if not hasattr(self, "sg") or not hasattr(self, "sgPrjId") or (user and not hasattr(self, "sgUserId")):
			...
			sgProjectName = self.core.getConfig('shotgun', "projectname", configPath=self.core.prismIni)
			sgScriptName = self.core.getConfig('shotgun', "scriptname", configPath=self.core.prismIni)
			sgApiKey = self.core.getConfig('shotgun', "apikey", configPath=self.core.prismIni)

			if sgSite is None or sgProjectName is None or sgScriptName is None or sgApiKey is None:
				QMessageBox.warning(self.core.messageParent,"Shotgun", "Not all required information for the authentification are configured.")
			...
			try:
				...
				QMessageBox.warning(self.core.messageParent,"Shotgun", "Could not request Shotgun data:\n\n%s" % e)
				return [None, None, None]

			if len(sgPrj) == 0:
				QMessageBox.warning(self.core.messageParent,"Shotgun", "Could not find project \"%s\" in Shotgun." % sgProjectName)
	...
	def sgPublish(self, origin):
		...
		sgp = ShotgunPublish.sgPublish(core=self.core, origin=self, ptype=pType, shotName=shotName, task=taskName, version=versionName, sources=imgPaths, startFrame=sf)
	...
	def sgAssetsToLocal(self, origin):
		...
		for i in sgAssets:
			if hasLocalField and i['sg_localhierarchy'] is not None:
				...
				assetPath = os.path.join(origin.aBasePath, i['sg_asset_type'], i['code'])
	...
	def sgShotsToSG(self, origin):
		...
		for shot in localShots:
			...
			shotTasks = {}
			for k in shotSteps:
				for m in os.walk(os.path.join(origin.sBasePath, shot[0], "Scenefiles", k)):
					shotTasks[k] = m[1]
					break
