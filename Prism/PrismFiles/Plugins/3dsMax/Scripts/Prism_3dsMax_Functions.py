...
class Prism_3dsMax_Functions(object):
	...
	def getImportPaths(self, origin):
		num = self.executeScript(origin, "fileProperties.findProperty #custom \"PrismImports\"")
		if num == 0:
			return False
		else:
			return self.executeScript(origin, "fileProperties.getPropertyValue #custom %s" % num)
	...
	def projectBrowserStartup(self, origin):
		if self.executeScript(origin, "getFileVersion \"$max/3dsmax.exe\"")[:2] in ["19", "20"]:
			origin.loadOiio()
	...
	def isNodeValid(self, origin, handle):
		return (not (handle is None or MaxPlus.INode.GetINodeByHandle(handle).GetUnwrappedPtr() is None))
	...
	def sm_export_setTaskText(self, origin, prevTaskName):
		origin.l_taskName2.setText(origin.nameWin.e_item.text())
...
""")
	...
	@err_decorator
	def sm_export_exportAppObjects(self, origin, startFrame, endFrame, outputName, scaledExport=False, expNodes=None):
		if startFrame == endFrame:
			self.setFrameRange(origin, startFrame = startFrame, endFrame = endFrame + 1)
		else:
			self.setFrameRange(origin, startFrame = startFrame, endFrame = endFrame)
		...
		if origin.cb_outType.currentText() == ".obj":
			for i in range(startFrame, endFrame+1):
				MaxPlus.Animation.SetTime(i*MaxPlus.Animation.GetTicksPerFrame(), False)
				foutputName = outputName.replace("####", format(i, '04'))
				if origin.chb_wholeScene.isChecked():
					MaxPlus.FileManager.Export(foutputName, not origin.chb_additionalOptions.isChecked())
				else:
					MaxPlus.FileManager.ExportSelected(foutputName, not origin.chb_additionalOptions.isChecked())
			...
			if startFrame == endFrame:
				...
				self.executeScript(origin, "FbxExporterSetParam \"Animation\" True", returnVal=False)
			if origin.chb_wholeScene.isChecked():
				MaxPlus.FileManager.Export(outputName, not origin.chb_additionalOptions.isChecked())
			else:
				MaxPlus.FileManager.ExportSelected(outputName, not origin.chb_additionalOptions.isChecked())
		...
		if scaledExport:
			...
			for i in expNodes:
				...
				nodes.Append(iNode)
				scaleHelper = MaxPlus.INode.GetINodeByName("SCALEOVERRIDE_" + iNode.GetName())
...
dialogMonitorOps.enabled = false""")
			...
			scaleNodes = [x for x in impNodes if (MaxPlus.INode.GetINodeByHandle(x)).GetParent().GetName() == "Scene Root"]
			for i in scaleNodes:
				self.executeScript(origin, """obj = (maxOps.getNodeByHandle %s)
sHelper = point()
...
sHelper.scale = [sVal, sVal, sVal]""" % i)
			...
			MaxPlus.Animation.SetTime(origin.sp_rangeStart.value()*MaxPlus.Animation.GetTicksPerFrame())
	#		for i in impNodes:
	#			self.executeScript(origin, "ResetXForm (maxOps.getNodeByHandle %s)" % i)
			...
			outputName = self.sm_export_exportAppObjects(origin, startFrame, endFrame, outputName, scaledExport=True, expNodes=impNodes)
	...
	def sm_export_preExecute(self, origin, startFrame, endFrame):
		...
		if origin.cb_outType.currentText() != "ShotCam":
			...
			if origin.chb_convertExport.isChecked():
				warnings.append(["Unit conversion is enabled.", "This causes a renaming of the exported converted objects.", 2])

		if origin.cb_outType.currentText() == ".fbx" or origin.cb_outType.currentText() == "ShotCam" and startFrame == endFrame:
			warnings.append(["Single frame FBX exports will export Frame 0 only.", "", 2])
	...
	def sm_render_isVray(self, origin):
		return self.executeScript(origin, "matchpattern (classof renderers.current as string) pattern: \"V_Ray*\"")
	...
	def sm_render_preSubmit(self, origin, rSettings):
		...
		MaxPlus.RenderElementMgr.SetElementsActive(elementMgr, origin.gb_passes.isChecked())
	...
	def sm_render_startLocalRender(self, origin, outputName, rSettings):
		...
		MaxPlus.RenderSettings.SetStart(jobFrames[0] * MaxPlus.Animation.GetTicksPerFrame())
		MaxPlus.RenderSettings.SetEnd(jobFrames[1] * MaxPlus.Animation.GetTicksPerFrame())
		
		self.executeScript(origin, "rendUseActiveView = True")
		if origin.curCam != "Current View":
			MaxPlus.Viewport.SetViewCamera(MaxPlus.ViewportManager.GetActiveViewport(), MaxPlus.INode.GetINodeByHandle(origin.curCam))
...
for mapfile in mapfiles collect mapfile")
	...
	def sm_render_getRenderPasses(self, origin):
		if self.sm_render_isVray(origin):
			return self.core.getConfig("defaultpasses", "max_vray", configPath=self.core.prismIni)
		else:
			return self.core.getConfig("defaultpasses", "max_scanline", configPath=self.core.prismIni)
	...
	def sm_import_importToApp(self, origin, doImport, update, impFileName):
		...
		if fileName[1] == ".max":
			...
			if not update or len(validNodes) == 0:
				msg = QMessageBox(QMessageBox.Question, "Create Reference", "Do you want to create a reference?", QMessageBox.No)
				msg.addButton("Yes", QMessageBox.YesRole)
				msg.setParent(self.core.messageParent, Qt.Window)
				action = msg.exec_()
			else:
				action = 1 - int(self.executeScript(origin, """
(
	...
	item != undefined
			...
			if action == 0:
				...
				if len(validNodes) > 0:
					isXref = self.executeScript(origin, """
(
	item = objXRefMgr.IsNodeXRefed (maxOps.getNodeByHandle %s)
	item != undefined
)""" % validNodes[0])
					...
					if isXref:
						createNewXref = False
						result = self.executeScript(origin, """
(
	item = objXRefMgr.IsNodeXRefed (maxOps.getNodeByHandle %s)
...
record != undefined\n\
				...
				if origin.chb_abcPath.isChecked():
					for i in origin.nodes:
						if not self.isNodeValid(origin, i):
							continue
						MaxPlus.INode.GetINodeByHandle(i).BaseObject.ParameterBlock.source.Value = impFileName
						result = True
						doImport = False
				self.executeScript(origin, "AlembicImport.ImportToRoot = True")
			elif fileName[1] == ".fbx":
			...
			if doImport:
				result = MaxPlus.FileManager.Import(impFileName, True)
...
""" % i)
			...
			QMessageBox.information(self.core.messageParent, "Unit conversion", "Please note that only vertex transformations will be converted. Object transformations and offset from origin are not supported.\nYou can delete the XForm modifier on the imported objects to undo the unit conversion.")
			for i in origin.nodes:
				if not self.isNodeValid(origin, i):
					continue
...
""" % (outputName.replace("\\", "\\\\"), jobFrames[0], jobFrames[1], jobFrames[0], jobFrames[1]))
	...
	def sm_playblast_preExecute(self, origin):
		...
		if jobFrames[0] == jobFrames[1]:
			warnings.append(["Playblasts in 3ds Max are only supported with at least two frames.", "", 3])
	...
	def sm_playblast_execute(self, origin):
		...
		if jobFrames[0] == jobFrames[1]:
			return [origin.state.text(0) + ": error - Playblasts in 3ds Max are only supported with at least two frames."]
	...
	def sm_startup(self, origin):
		...
		if not origin.core.smCallbacksRegistered:
			MaxPlus.NotificationManager.Register(MaxPlus.NotificationCodes.FilePostSave , origin.core.scenefileSaved)
			MaxPlus.NotificationManager.Register(MaxPlus.NotificationCodes.PostSceneReset , origin.core.sceneUnload)
			MaxPlus.NotificationManager.Register(MaxPlus.NotificationCodes.FilePreOpen , origin.core.sceneUnload)
	...
	def sm_saveImports(self, origin, importPaths):
		self.executeScript(origin, "fileProperties.addProperty #custom \"PrismImports\" \"%s\"" % importPaths)


	@err_decorator
	def sm_readStates(self, origin):
		num = self.executeScript(origin, "fileProperties.findProperty #custom \"PrismStates\"")
		if num != 0:
			return self.executeScript(origin, "fileProperties.getPropertyValue #custom %s" % num)
	...
	def sm_deleteStates(self, origin):
		num = self.executeScript(origin, "fileProperties.findProperty #custom \"PrismStates\"")
		if num != 0:
			self.executeScript(origin, "fileProperties.deleteProperty #custom \"PrismStates\"")
