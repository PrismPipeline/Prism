...
class Prism_Blender_Functions(object):
	...
	def sm_export_addObjects(self, origin):
		if origin.l_taskName2.text() not in bpy.data.groups:
			bpy.ops.group.create(name=origin.l_taskName2.text())

		for i in [ o for o in bpy.context.scene.objects if o.select and o not in list(bpy.data.groups[origin.l_taskName2.text()].objects) ]:
			bpy.data.groups[origin.l_taskName2.text()].objects.link(i)
	...
	def sm_export_startup(self, origin):
		...
		if not origin.stateManager.loading:
			...
			origin.l_taskName2.setText(setName)
	...
	def sm_export_removeSetItem(self, origin, node):
		...
		bpy.data.groups[origin.l_taskName2.text()].objects.unlink(bpy.data.objects[node])
	...
	def sm_export_clearSet(self, origin):
		...
		for node in bpy.data.groups[origin.l_taskName2.text()].objects:
			bpy.data.groups[origin.l_taskName2.text()].objects.unlink(node)


	@err_decorator
	def sm_export_updateObjects(self, origin):
		origin.nodes = []
		if origin.l_taskName2.text() in bpy.data.groups:
			origin.nodes = bpy.data.groups[origin.l_taskName2.text()].objects.keys()


	@err_decorator
	def sm_export_exportShotcam(self, origin, startFrame, endFrame, outputName):
		self.selectCam(origin)
		bpy.ops.wm.alembic_export(self.getOverrideContext(origin), filepath=(outputName + ".abc"), start=startFrame, end=endFrame, selected=True, as_background_job=False)
		self.selectCam(origin)
		bpy.ops.export_scene.fbx(self.getOverrideContext(origin), filepath=(outputName + ".fbx"), use_selection=True)

		if origin.chb_convertExport.isChecked(): # disabled because of a blender bug (rescales animated object back to 1 on export)
	...
	def sm_export_exportAppObjects(self, origin, startFrame, endFrame, outputName, scaledExport=False, expNodes=None):
		...
		if origin.cb_outType.currentText() == ".obj":
			for i in range(startFrame, endFrame+1):
				...
				bpy.ops.export_scene.obj(self.getOverrideContext(origin), filepath=foutputName, use_selection=(not origin.chb_wholeScene.isChecked()))
	...
	#					impNodes.append(i.name)
		...
		#	bpy.ops.object.transform_apply(self.getOverrideContext(origin), location=True, rotation=True, scale=True)
			...
			outputName = self.sm_export_exportAppObjects(origin, startFrame, endFrame, outputName, scaledExport=True, expNodes=expNodes)
					...
					if i.to_node.type == "OUTPUT_FILE":
						for idx, k in enumerate(i.to_node.file_slots):
							...
							if len(links) > 0:
								...
								if passName == itemName.split("_",1)[1] and layerName == itemName.split("_",1)[0]:
	...
	def sm_render_startLocalRender(self, origin, outputName, rSettings):
		...
		try:
			if not origin.renderingStarted:
				origin.waitmsg = QMessageBox(QMessageBox.NoIcon, "ImageRender", "Local rendering - %s - please wait.." % origin.state.text(0), QMessageBox.Cancel)
	...
	def sm_render_fixOutputPath(self, origin, outputName):
		outputName = os.path.splitext(outputName)[0] + "####" + os.path.splitext(outputName)[1]
		return outputName
	...
	def sm_import_importToApp(self, origin, doImport, update, impFileName):
		...
		if fileName[1] in [".blend"]:
			...
			if action == 0:
				if len(validNodes) > 0 and bpy.data.objects[validNodes[0]].library is not None:
					updateObjs = True
					for i in validNodes:
			...
			if fileName[1] not in [".fbx", ".obj", ".abc"]:
				QMessageBox.warning(self.core.messageParent, "ImportFile", "Format is not supported.")
			...
			if fileName[1] == ".fbx":
				...
				if origin.chb_abcPath.isChecked() and len(origin.nodes) > 0:
					...
					for i in origin.nodes:
						constraints = [x for x in bpy.data.objects[i].constraints if x.type == "TRANSFORM_CACHE"]
						modifiers = [x for x in bpy.data.objects[i].modifiers if x.type == "MESH_SEQUENCE_CACHE"]
				...
				#		bpy.context.scene.frame_current -= 1
						...
						QMessageBox.warning(self.core.messageParent, "ImportFile", "No caches updated.")
					result = True
				else:
	...
	def sm_playblast_createPlayblast(self, origin, jobFrames, outputName):
		...
		prevRes = [bpy.context.scene.render.resolution_x, bpy.context.scene.render.resolution_y, bpy.context.scene.render.resolution_percentage]
		prevOutput = [bpy.context.scene.render.filepath, bpy.context.scene.render.image_settings.file_format]
		...
		bpy.ops.render.opengl(self.getOverrideContext(origin), animation=renderAnim, write_still=True)
