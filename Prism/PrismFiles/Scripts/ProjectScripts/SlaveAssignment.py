...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "UserInterfaces"))
...
class SlaveAssignment(QDialog, SlaveAssignment_ui.Ui_dlg_SlaveAssignment):
	...
	def getSlaves(self):
		...
		if platform.system() == "Windows":
			configPath = os.path.join(os.getenv('LocalAppdata'), "Pandora", "Config", "Pandora.ini")
		else:
			return False
		...
		if not pconfig.has_option("submissions", "submissionpath"):
			QMessageBox.warning(self,"Warning", "No Pandora job submissions folder specified in the Pandora config")
			return False

		slaveDir = os.path.join(os.path.dirname(os.path.dirname(pconfig.get('submissions', "submissionpath"))), "Logs", "Slaves")
