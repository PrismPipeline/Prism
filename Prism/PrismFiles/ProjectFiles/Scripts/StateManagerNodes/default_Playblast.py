...
class PlayblastClass(object):
	...
	def loadData(self, data):
		...
		if "currentcam" in data:
			idx = self.cb_cams.findText(self.core.plugin.getCamName(self, data["currentcam"]))
			if idx > 0:
	...
	def updateUi(self):
		...
		self.cb_cams.addItems([self.core.plugin.getCamName(self, i) for i in self.camlist])
	...
	def preExecuteState(self):
		...
		warnings += self.core.plugin.sm_playblast_preExecute(self)
	...
	def executeState(self, parent, useVersion="next"):
		...
		exCheck = self.core.plugin.sm_playblast_execute(self)
