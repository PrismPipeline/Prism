...
class ImageRenderClass(object):
	...
	def setup(self, state, core, stateManager, node=None, stateData=None):
		...
		self.core.plugin.sm_render_startup(self)

		if not self.core.plugin.sm_render_isVray(self):
		...
		self.core.plugin.sm_render_setTaskWarn(self, True)
	...
	def loadData(self, data):
		if "taskname" in data:
			...
			if data["taskname"] != "":
				self.core.plugin.sm_render_setTaskWarn(self, False)
	...
	def connectEvents(self):
		...
		self.lw_passes.itemDoubleClicked.connect(lambda x: self.core.plugin.sm_render_openPasses(self))
	...
	def updateUi(self):
		...
		self.camlist = self.core.plugin.getCamNodes(self, cur=True)
		...
		layerList = getattr(self.core.plugin, "sm_render_getRenderLayer", lambda x: [])(self)
		...
		self.core.plugin.sm_render_refreshPasses(self)
	...
	@err_decorator
	def showPasses(self):
		steps = self.core.plugin.sm_render_getRenderPasses(self)
	...
	def rclickPasses(self, pos):
		if self.lw_passes.currentItem() is None or not getattr(self.core.plugin, "canDeleteRenderPasses", True):
			return
		...
		delAct.triggered.connect(lambda: self.core.plugin.sm_render_deletePass(self, item))
	...
	def preExecuteState(self):
		...
		if self.curCam is None or (self.curCam != "Current View" and not self.core.plugin.isNodeValid(self, self.curCam)):
		...
		warnings += self.core.plugin.sm_render_preExecute(self)
