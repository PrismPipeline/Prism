...
class ExportClass(object):
	...
	def setup(self, state, core, stateManager, node=None, stateData=None):
		...
		if not self.stateManager.loading:
			self.core.plugin.sm_export_addObjects(self)

		if stateData is not None:
			self.loadData(stateData)
		else:
			startFrame = self.core.plugin.getFrameRange(self)[0]
	...
	def loadData(self, data):
		...
		if "currentcam" in data:
			idx = self.cb_cam.findText(self.core.plugin.getCamName(self, data["currentcam"]))
			if idx != -1:
	...
	def connectEvents(self):
		...
		self.lw_objects.itemSelectionChanged.connect(lambda: self.core.plugin.selectNodes(self))
		self.lw_objects.customContextMenuRequested.connect(self.rcObjects)
		self.b_add.clicked.connect(lambda: self.core.plugin.sm_export_addObjects(self))
		self.cb_cam.activated.connect(self.setCam)
		self.cb_sCamShot.activated.connect(self.stateManager.saveStatesToScene)
		self.b_selectCam.clicked.connect(lambda: self.core.plugin.selectCam(self))
	...
	def nameChanged(self, text):
		if self.cb_outType.currentText() == "ShotCam":
			...
			taskname = self.l_taskName2.text()
			if taskname == "":
				taskname = "None"
	...
	def changeTask(self):
		...
		prevTaskName = self.l_taskName2.text()
		result = self.nameWin.exec_()
		
		if result == 1:
			self.core.plugin.sm_export_setTaskText(self, prevTaskName)
			self.b_changeTask.setPalette(self.oldPalette)
			self.nameChanged(self.e_name.text())
			self.stateManager.saveStatesToScene()
	...
	def removeItem(self, item):
		...
		for i in reversed(self.lw_objects.selectedItems()):
			...
			self.core.plugin.sm_export_removeSetItem(self, self.nodes[rowNum])
	...
	def clearItems(self):
		...
		self.nodes = []
		self.core.plugin.sm_export_clearSet(self)
	...
	def updateUi(self):
		self.cb_cam.clear()
		self.camlist = self.core.plugin.getCamNodes(self)
		self.cb_cam.addItems([self.core.plugin.getCamName(self, i) for i in self.camlist])
		...
		self.core.plugin.sm_export_updateObjects(self)

		for node in self.nodes:
			if self.core.plugin.isNodeValid(self, node):
				item = QListWidgetItem(self.core.plugin.getNodeName(self, node))
		...
		if self.lw_objects.count() == 0 and not self.chb_wholeScene.isChecked():
			getattr(self.core.plugin, "sm_export_colorObjList", lambda x: self.lw_objects.setStyleSheet("QListWidget { border: 3px solid rgb(200,0,0); }") )(self)
		else:
			getattr(self.core.plugin, "sm_export_unColorObjList", lambda x: self.lw_objects.setStyleSheet("QListWidget { border: 3px solid rgb(114,114,114); }") )(self)
	...
	def typeChanged(self, idx):
		...
		getattr(self.core.plugin, "sm_export_typeChanged", lambda x, y: None)(self, idx)
	...
	def preExecuteState(self):
		...
		warnings += self.core.plugin.sm_export_preExecute(self, startFrame, endFrame)
	...
	def getOutputName(self, useVersion="next", startFrame=0, endFrame=0):
		prefUnit = self.core.plugin.preferredUnit
	...
	def executeState(self, parent, useVersion="next"):
		...
		if self.cb_outType.currentText() == "ShotCam":
			...
			if not self.chb_wholeScene.isChecked() and len([x for x in self.nodes if self.core.plugin.isNodeValid(self, x)]) == 0:
			...
			try:
				outputName = self.core.plugin.sm_export_exportAppObjects(self, startFrame=startFrame, endFrame=endFrame, outputName=outputName)
