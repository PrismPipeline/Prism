...
class ImportFileClass(object):
	...
	def setup(self, state, core, stateManager, node=None, importPath=None, stateData=None):
		...
		self.core.plugin.sm_import_startup(self)
		self.connectEvents()
	...
	def loadData(self, data):
		...
		if "filepath" in data:
			data["filepath"] = getattr(self.core.plugin, "sm_import_fixImportPath", lambda x,y:y)(self, data["filepath"])
		...
		if "connectednodes" in data:
			self.nodes = [x[1] for x in eval(data["connectednodes"]) if self.core.plugin.isNodeValid(self, x[1])]
		if "taskname" in data:
	...
	def connectEvents(self):
		...
		self.b_nameSpaces.clicked.connect(lambda: self.core.plugin.sm_import_removeNameSpaces(self))
		self.b_unitConversion.clicked.connect(lambda: self.core.plugin.sm_import_unitConvert(self))
	...
	def nameChanged(self, text):
		getattr(self.core.plugin, "sm_import_nameChanged", lambda x:None)(self)
	...
	def autoNameSpaceChanged(self, checked):
		...
		self.core.plugin.sm_import_removeNameSpaces(self)
		self.stateManager.saveStatesToScene()
	...
	def importObject(self, taskName=None, update=False):
		if self.e_file.text() != "":
			...
			if doImport:
				self.nodeNames = [self.core.plugin.getNodeName(self, x) for x in self.nodes]
		...
		if os.path.exists(versionPath):
			...
			for i in os.walk(versionPath):
				if len(i[2]) > 0:
					...
					if getattr(self.core.plugin, "shotcamFormat", ".abc") == ".fbx"  and self.taskName == "ShotCam" and fileName.endswith(".abc") and os.path.exists(fileName[:-3] + "fbx"):
	...
	def updateUi(self):
		if os.path.exists(self.e_file.text()):
			...
			useSS = getattr(self.core.plugin, "colorButtonWithStyleSheet", False)
		...
		for i in self.nodes:
			item = QListWidgetItem(self.core.plugin.getNodeName(self, i))
			getattr(self.core.plugin, "sm_import_updateListItem", lambda x,y, z:None)(self, item, i)
	...
	def updatePrefUnits(self):
		pref = self.core.plugin.preferredUnit
			...
			if len(validNodes) > 0:
				for idx, val in enumerate(validNodes):
					if idx > 5:
						...
						message += self.core.plugin.getNodeName(self, val) + "\n"
				...
				if action == 0:
					self.core.plugin.deleteNodes(self, validNodes)
