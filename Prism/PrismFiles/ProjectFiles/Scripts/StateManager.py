...
class StateManager(QMainWindow, StateManager_ui.Ui_mw_StateManager):
	def __init__(self, core, stateDataPath=None, getVersion=False):
		...
		files = []
		pluginUiPath = os.path.join(self.core.pluginPath, self.core.plugin.appName, "Scripts", "StateManagerNodes", "StateUserInterfaces")
		...
		sys.path.append(os.path.join(os.path.dirname(self.core.prismIni), "Scripts", "StateManagerNodes"))
		sys.path.append(os.path.join(os.path.dirname(self.core.prismIni), "Scripts", "StateManagerNodes", "StateUserInterfaces"))
		...
		for i in files:
			try:
				if os.path.splitext(i)[1] != ".pyc" or (os.path.splitext(i)[1] == ".pyc" and not os.path.exists(os.path.splitext(i)[0] + ".py") and i != "__init__.pyc"):
					...
					if stateName.startswith("default_") or stateName.startswith(self.core.plugin.appShortName.lower()):
...
class %s(QWidget, %s.%s, %s.%sClass):
	def __init__(self):
		...
		self.core.plugin.sm_startup(self)
		self.loadLayout()
	...
	def loadLayout(self):
		...
		self.core.plugin.setRCStyle(self, helpMenu)
	...
	def setTreePalette(self, listWidget, inactive, inactivef, activef):
		listWidget.setStyleSheet("QTreeWidget { border: 1px solid rgb(150,150,150); }")
	...
	def loadStates(self, stateText=None):
		...
		if stateText is None:
			stateText = self.core.plugin.sm_readStates(self)
	...
	def setListActive(self, listWidget):
		...
		getattr(self.core.plugin, "sm_setActivePalette", lambda x1,x2,x3,x4,x5: self.setTreePalette(x2,x3,x4,x5))(self, listWidget, inactive, inactivef, activef)
	...
	def connectEvents(self):
		...
		self.b_createRender.clicked.connect(lambda: self.core.plugin.sm_createRenderPressed(self))
		...
		self.b_stateFromNode.clicked.connect(lambda: self.core.plugin.sm_openStateFromNode(self))
	...
	def showStateList(self):
		...
		self.core.plugin.setRCStyle(self, createMenu)
	...
	def copyAllStates(self):
		stateData = self.core.plugin.sm_readStates(self)
	...
	def removeAllStates(self):
		...
		self.core.plugin.sm_deleteStates(self)
	...
	def shotCam(self):
		...
		if "mCamState" in locals():
			...
			camPath = os.path.join(camPath, highFolder, self.core.plugin.preferredUnit)
	...
	def saveStatesToScene(self, param=None):
		...
		getattr(self.core.plugin, "sm_preSaveToScene", lambda x: None)(self)
	...
	#	print "save to scene"
		...
		self.core.plugin.sm_saveStates(self, buf.getvalue())


	@err_decorator
	def saveImports(self):
		importPaths = str(self.getFilePaths(self.tw_import.invisibleRootItem(), []))
		self.core.plugin.sm_saveImports(self, importPaths)
	...
	def publish(self, executeState=False, continuePublish=False, useVersion="next"):
		...
		if continuePublish:
			...
			for idx, i in enumerate(extFiles):
				...
				if not os.path.exists(i) and not i in nonExistend and i != self.core.getCurrentFileName():
					exists = getattr(self.core.plugin, "sm_existExternalAsset", lambda x,y:False)(self, i)
					if exists:
						continue
			...
			if len(invalidFiles) > 0:
				...
				for i in invalidFiles:
					parmStr = getattr(self.core.plugin, "sm_fixWarning", lambda x1,x2,x3,x4: "")(self, i, extFiles, extFilesSource)
			...
			getattr(self.core.plugin, "sm_preExecute", lambda x:None)(self)
		...
		getattr(self.core.plugin, "sm_postExecute", lambda x:None)(self)
