...
class ProjectBrowser(QMainWindow, ProjectBrowser_ui.Ui_mw_ProjectBrowser):
	def __init__(self, core, getVersion=False):
	...
	#	self.core.reloadPlugins()
		...
		self.setAppMethods()
		getattr(self.core.plugin, "projectBrower_loadLibs", lambda x: self.loadLibs())(self)
	...
	def setAppMethods(self):
		self.startup = self.core.plugin.projectBrowserStartup
		self.loadAppLayout = self.core.plugin.projectBrowserLoadLayout
		self.setRCStyle = self.core.plugin.setRCStyle
		self.openSceneFile = self.core.plugin.openScene
		self.correctExt = self.core.plugin.correctExt
		self.setSaveColor = self.core.plugin.setSaveColor
		self.clearSaveColor = self.core.plugin.clearSaveColor
		...
		if self.tbw_browser.tabText(tab) == "Assets":
			...
			if curAssetItem is None:
				...
				if curStepItem is None:
					...
					curStep = os.path.join("Scenefiles", "step_" + curStepItem.text())
	...
	def rclFile(self, tab, pos):
		if tab == "a":
			...
			tw = self.tw_aFiles
			filepathrow = 5
			filepath = os.path.join(self.tw_aHierarchy.currentItem().text(1), "Scenefiles", "step_" + self.lw_aPipeline.currentItem().text())
		elif tab == "sf":
		...
		if tab != "r":
			...
			if self.core.plugin.appName == "Standalone":
				current.setEnabled(False)
			rcmenu.addAction(current)
			emp = QMenu("Create empty file")
			emptyDir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "EmptyScenes")
	...
	def exeFile(self, index=None, filepath=""):
		...
		isOpen = self.openSceneFile(self, filepath)

		if not isOpen and self.core.plugin.appName == "Standalone":
			fileStarted = False
			...
			for i in self.core.unloadedPlugins:
				if ext in i.sceneFormats:
					orApp = self.core.getConfig("dccoverrides", "%s_override" % i.appName.lower(), ptype="bool")
					if orApp is not None and orApp:
						appOrPath = self.core.getConfig("dccoverrides", "%s_path" % i.appName.lower())
	...
	def autoback(self, tab, prog):
		if prog == self.core.plugin.appName:
			autobackpath, fileStr = self.core.plugin.getAutobackPath(self, tab)
		else:
			for i in self.core.unloadedPlugins:
				if i.appName == prog:
	...
	def createEmpty(self, tab, fname):
		if fname == "createnew":
			...
			newItem = CreateItem.CreateItem(core=self.core, startText=self.core.plugin.appName.replace(" ", ""))
			...
			if result == 1:
				...
				filepath = filepath.replace("\\","/")
				filepath += self.core.plugin.getSceneExtension(self)
		...
		if "newfname" in locals():
			...
			if ext in self.core.plugin.sceneFormats:
				getattr(self.core.plugin, "preLoadEmptyScene", lambda x, y: None)(self, filepath)
				self.exeFile(filepath=filepath)
				getattr(self.core.plugin, "postLoadEmptyScene", lambda x, y: None)(self, filepath)
			else:
				self.core.addToRecent(filepath)
				self.setRecent()
				refresh()
	...
	def pastefile(self, tab):
		if tab == "a":
			...
			if os.path.splitext(dstname)[1] in self.core.plugin.sceneFormats:
	...
	def refreshAHierarchy(self, load=False):
		...
		for path in dirs:
			...
			if val[:5] != "step_" and val not in self.omittedEntities["Asset"]:
	...
	def refreshAFile(self, cur=None, prev=None):
		...
		if self.tw_aHierarchy.currentItem() is not None and self.lw_aPipeline.currentItem() is not None:
			path = os.path.join(self.tw_aHierarchy.currentItem().text(1), "Scenefiles", "step_" + self.lw_aPipeline.currentItem().text())
		...
		for i in scenefiles:
			...
			if len(fname) == 6 and fname[5] in appfilter:
				...
				if ext in self.core.plugin.sceneFormats:
					colorVals = self.core.plugin.appColor
				else:
					for k in self.core.unloadedPlugins:
	...
	def createStep(self, tab):
		...
		steps = {validSteps : steps[validSteps] for validSteps in steps if not os.path.exists(os.path.join(basePath, prefix + validSteps))}
		steps = self.getStep(steps, tab)
		if steps != False:
			createdDirs = []
			for i in steps[0]:
				if tab == "a":
					dstname = os.path.join(basePath, prefix + i)
				elif tab == "s":
	...
	def navigateToCurrent(self, path=None):
		...
		if os.path.join(self.core.projectPath, self.scenes) in fileName or (self.core.useLocalFiles and os.path.join(self.core.localProjectPath, self.scenes) in fileName):
			if len(fileNameData) == 6 or self.aBasePath in fileName or (self.core.useLocalFiles and (self.aBasePath.replace(self.core.projectPath, self.core.localProjectPath) in fileName)):
				...
				itemPath = itemPath.replace(os.sep + os.path.join( self.scenes, os.path.join("Assets", "Scenefiles", "")), "")
					...
					for idx, i in enumerate((hierarchy[1:])):
						for k in range(hItem.childCount()-1,-1,-1):
							if hItem.child(k).text(0) == i:
								hItem = hItem.child(k)
								if len(hierarchy) > (idx+2) and not hierarchy[idx+2].startswith("step_"):
									hItem.setExpanded(True)
									if hItem.text(1) not in self.aExpanded:
										self.aExpanded.append(hItem.text(1))
								break
				...
				if endIdx is not None and hierarchy[endIdx].startswith("step_"):
					fItems = self.lw_aPipeline.findItems(hierarchy[endIdx][len("step_"):], Qt.MatchExactly)
					if len(fItems) > 0:
		...
		if self.curRLayer != "":
			...
			if os.path.exists(redirectFile):
				...
				if os.path.splitext(rpath)[1] == "":
					...
					foldercont = [os.path.dirname(rpath), [], [os.path.basename(rpath)]]
	...
	def rclPreview(self, pos):
		...
		if self.core.plugin.appType == "2d" and len(self.seq) > 0:
			impAct = QAction("Import images...", self)
			impAct.triggered.connect(lambda: self.core.plugin.importImages(self))
	...
	def sendToDailies(self):
		...
		if platform.system() == "Windows":
			...
			folderBat = os.path.join(dailiesFolder, batFolderName)

			with open(seqBat, "w") as seqFile:
				cmd = "start \"\" \"%s\"" % sourcePath
				seqFile.write(cmd)

			with open(folderBat, "w") as folderFile:
				cmd = "explorer \"%s\"" % self.basepath
				folderFile.write(cmd)
		else:
		...
		QMessageBox.information(self.core.messageParent, "Dailies", "The reference was sent to the current dailies folder. (path in clipboard)")
	...
	def convertImgs(self, extension):
		...
		if platform.system() == "Windows":
			...
			ffmpegPath = os.path.join(os.path.dirname(self.core.userini), "Tools", "ffmpeg")
			if os.path.exists(ffmpegPath):
				ffmpegIsInstalled = True
...
if __name__ == "__main__":
	...
	pc.changeProject(os.path.abspath(os.path.join(__file__, os.pardir, os.pardir, "pipeline.ini")))
	pc.projectBrowser()
	sys.exit(qapp.exec_())