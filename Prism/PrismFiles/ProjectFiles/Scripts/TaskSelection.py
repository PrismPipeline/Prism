...
class TaskSelection(QDialog, TaskSelection_ui.Ui_dlg_TaskSelection):
	...
	def openCustom(self):
		...
		fileName = customFile
		impPath = getattr(self.core.plugin, "fixImportPath", lambda x, y:y)(self, splitName[0])
	...
	def updateShots(self):
		...
		for path in dirs:
			...
			if val[:5] == "step_" or val.startswith("_") or val in omittedShots:
				continue
	...
	def updateVersions(self):
		...
		if self.curEntity is not None and self.curTask is not None:
			...
			for k in versions:
				...
				for n, unit in enumerate(["centimeter", "meter", ""]):
					for m in os.walk(os.path.join(k[1], k[0], unit)):
						if len(m[2]) > 0 and os.path.splitext(m[2][0])[1] not in [".txt", ".ini"]:
							...
							if getattr(self.core.plugin, "shotcamFormat", ".abc") == ".fbx" and self.curTask == "ShotCam" and fileName[n].endswith(".abc") and os.path.exists(fileName[n][:-3] + "fbx"):
				...
				impPath = getattr(self.core.plugin, "fixImportPath", lambda x, y:y)(self, depPath)
